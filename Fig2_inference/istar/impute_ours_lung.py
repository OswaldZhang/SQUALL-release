...
import argparse
from extract_features import smoothen_embeddings
...
from image import get_disk_mask,smoothen
...
class SpotDataset(Dataset):
    ...
    def __init__(self, x_all, y, locs, radius):
        ...
        print(f"x_all.shape={x_all.shape}, y.shape={y.shape}, locs.shape={locs.shape}",flush=True)
        #x = get_patches_flat(x_all, locs, mask)
        x, new_locs = get_patches_flat(x_all, locs, mask)
...
def get_embeddings_ours(prefix):
    ...
    data = torch.load("/lustre1/zxzeng/bwqin/SQUALL/Xenium/Lung_Xenium_public/lung_embeddings_all_rgb_only.pt",map_location=torch.device('cpu'))
    ...
    max_x, max_y  = data['dimensions']
    print("max_x, max_y",max_x, max_y)
    target_width = int(max_y/16)  # 计算目标图的高度
    target_height = int(max_x/16)  # 计算目标图的宽度
    embs = embs.permute(2, 0, 1).unsqueeze(0)[:,:,:target_height,:target_width]#1CHW
    print("embs.shape",embs.shape)
    #print(H_target//embs.shape[2])
    #print(W_target//embs.shape[3])
    #H_target_up = ((H_target//embs.shape[2])+1)*embs.shape[2]
    #W_target_up = ((W_target//embs.shape[3])+1)*embs.shape[3]
    embs_upsampled = F.interpolate(embs, size=(H_target, W_target), mode='bilinear', align_corners=True)
    embs_upsampled = embs_upsampled.squeeze(0).permute(1, 2, 0)  # [H, W, C]
    embs_upsampled_np = embs_upsampled.cpu().numpy()
    #print("black",compute_black_area(embs_upsampled_np))
    print("embs_upsampled_np",embs_upsampled_np.shape,flush = True)
    embs_upsampled_np = smoothen(
                embs_upsampled_np, size=4, kernel='uniform',
                backend='torch',
                device='cpu')
    save_embedding_heatmap(embs_upsampled_np)
    return embs_upsampled_np

def get_data(prefix):
    gene_names = read_lines(f'{prefix}gene-names.txt')
    delete = ['ZNF395', 'ZFAND5', 'ZCCHC10', 'WASHC2A', 'VPS35', 'TUBB4A', 'TUBB3', 'TJP2', 'TDGF1', 'TARDBP', 'SKP1', 'RBMS2', 'RACGAP1', 'PTCD3', 'PRODH', 'POLR2M', 'PNPT1', 'PDXDC1', 'PDE4DIP', 'PARK7', 'NCF1', 'IMPDH1', 'IL6ST', 'IGF2', 'HS3ST3A1', 'GUCA1A', 'GTF2A1L', 'ELOB', 'DRG1', 'DPY19L2', 'CCT5', 'CBX1', 'CBS', 'APLF', 'ABCC6']
    gene_names = [i for i in gene_names if i not in delete]
    cnts = get_gene_counts(prefix)
    cnts = cnts[gene_names]
    #embs = get_embeddings(prefix)
    embs = get_embeddings_ours(prefix)
