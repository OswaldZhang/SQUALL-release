            ...
            if pathlib.Path(complete_filename).exists():
                ...
                for (_, row) in raw[patient][section]["spot"].iterrows():
                    ...
                    if X.shape == (window, window, 3):
                        if (int(round(row["x"])), int(round(row["y"]))) in tumor:
                            data.append((X,count[str(row['barcode'])],
                                #count[str(int(row["x"])) + "x" + str(int(row["y"]))],
                                         tumor[(int(round(row["x"])), int(round(row["y"])))],
                                         np.array([x, y]),
                                         np.array([patient]),
                                         np.array([section]),
                                         np.array([int(round(row["x"])), int(round(row["y"]))]),
                                         ))
                            filename = "{}/{}/{}/{}_{}_{}.npz".format(args.dest, subtype[patient], patient, section,
                                                                      int(round(row["x"])), int(round(row["y"])))
                            np.savez_compressed(filename, count = count[str(row['barcode'])],#count=count[str(int(row["x"])) + "x" + str(int(row["y"]))],
                                                tumor=tumor[(int(round(row["x"])), int(round(row["y"])))],
                                                pixel=np.array([x, y]),
                                                patient=np.array([patient]),
                                                section=np.array([section]),
                                                index=np.array([int(round(row["x"])), int(round(row["y"]))]))
                        else:
                            logger.warning("Patch " + str(int(round(row["x"]))) + "x" + str(
                                int(round(row["y"]))) + " not found in " + patient + " " + section)
                    else:
