...
CUDA_VISIBLE_DEVICES=0 python main_hdgenes.py \
  --mode train_predict \
  --tile_root ${TILE_ROOT} \
  --train_sample OV_VisiumHD_all_new \
  --test_sample OV_Xenium_all_new \
  --out_dir outputs/EGNv1_HD_original_OV_to_OVXenium \
  ...
  --knn_batch 512 \
  --log_mode log10 \
  2>&1 | tee outputs/EGNv1_HD_original_OV_to_OVXenium.log
