...
CUDA_VISIBLE_DEVICES=3 python main_hdgenes.py \
  --mode predict_only \
  --tile_root ${TILE_ROOT} \
  --train_sample OV_VisiumHD_all_new \
  --test_sample CC_all_new \
  --out_dir outputs/EGNv1_HD_original_OVmodel_to_CC \
  ...
  --numk 16 \
  --knn_batch 512 \
  2>&1 | tee outputs/EGNv1_HD_original_OVmodel_to_CC.log
