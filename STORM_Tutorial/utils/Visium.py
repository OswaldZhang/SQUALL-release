...
def set_gene_token(adata,key,token_path):
    ...
    print("tkn_list",tkn_list)
    print("key",key)
    print("tkn_list['ENSMUSG_ID']",tkn_list['ENSMUSG_ID'])
...
def adata_token(adata,gene_token,tkn_info,key):
    ...
    try:
        ...
        print(f"Warning: Failed to make var_names unique: {e}")
...
def get_paths(folder_path):
    ...
    for file in os.listdir(os.path.join(folder_path,"spatial")):
        ...
        if ("hires" in file or (any(ext in file for ext in ['.tif', '.png','.jpg']) and 'lowres' not in file)):
        # Store file path and size for comparison
            file_size = os.path.getsize(file_path)
            potential_img_files.append((file_path, file_size))
    if potential_img_files:
        raw_img_path = min(potential_img_files, key=lambda x: x[1])[0]
        print('We use this image:', raw_img_path)
